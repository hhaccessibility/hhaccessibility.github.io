...
class Test
{
    public static function convertData(string $tableName)
    {
        ...
        foreach ($fieldNames as $fieldName) {
            $queryTable->update(['new_'.$fieldName => DB::raw(
                "IF(".$fieldName.
                    " is null, null, concat('00000000-0000-0000-0000-', LPAD(".
                    $fieldName.", 12, '0')))"
            )
            ]);
        }
    ...
    public static function reverseConvertData(string $tableName)
    {
        ...
        foreach ($fieldNames as $fieldName) {
            ...
            $queryTable->update([$newFieldName => DB::raw(
                "IF(".$fieldName." is null, null, CONVERT(SUBSTRING($fieldName,28),UNSIGNED INTEGER))"
            )
            ]);
        }
    }
